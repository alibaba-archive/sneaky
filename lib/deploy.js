...
(function() {
  var Deploy, Moment, async, deploy, exec, execCmd, expandPath, fs, ini, logger, mkdirp, path, sep1, sep2, spawn, spawnCmd, _, _ref,
  ...
  Deploy = (function() {
    ...
    Deploy.prototype.loadConfigs = function(callback) {
      ...
      return fs.readFile(configFile, function(err, content) {
        var configs, k, projectPrefix, v, _configs, _project;
        if (err != null) {
    ...
    Deploy.prototype.getServers = function(project) {
      var i, item, port, server, servers, user, _ref1, _ref2, _ref3;
      servers = [];
      if (typeof project.servers === 'string') {
        _ref1 = project.servers.split('|'), server = _ref1[0], user = _ref1[1], port = _ref1[2];
        user = user || this.configs.user || 'root';
        port = port || '22';
    ...
    Deploy.prototype.readRecords = function() {
      ...
      try {
        this.records = require(recordDir, "" + (moment.format('YYYYMMDD')) + ".json");
      } catch (_error) {
        e = _error;
        this.records = {};
      }
