...
(function() {
  var Logger, async, exec, fs, jsYaml, localDir, main, quit, readline;
  ...
  main = function(options, callback) {
    var after, archive, before, configs, deploy, logger, runCmd, ssh, start;
    if (options == null) {
      options = {};
    }
    if (callback == null) {
    ...
    deploy = function(project, callback) {
      ...
      return async.waterfall([
        (function(next) {
          ...
          var servers;
          servers = ssh(project);
          return async.eachSeries(servers, (function(server, _next) {
            var rsyncCmd;
            rsyncCmd = project.rsyncCmd || "rsync -a --timeout=15 --delete-after --ignore-errors --force " + ("" + localDir + "/" + project.name + " " + server[0] + "@" + server[1] + ":" + project.destination);
