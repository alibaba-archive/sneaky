...
(function() {
  var Logger, actionLogger, after, archive, async, before, configs, deploy, exec, fs, getServers, jsYaml, localDir, logger, main, moment, quit, rsync, runCmd;
  ...
  rsync = function(project, callback) {
    ...
    return async.eachSeries(servers, (function(server, next) {
      var rsyncCmd;
      rsyncCmd = project.rsyncCmd || "rsync -a --timeout=15 --delete-after --ignore-errors --force" + (" -e \"ssh -p " + server[2] + "\" ") + excludes.join(' ') + (" " + localDir + "/" + project.name + " " + server[1] + "@" + server[0] + ":" + project.destination);
