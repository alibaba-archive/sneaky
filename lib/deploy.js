...
(function() {
  ...
  main = function(options, callback) {
    var archive, before, configs, deploy, logger, runCmd, ssh, start;
    if (options == null) {
      options = {};
    }
    if (callback == null) {
      ...
      return async.waterfall([
        (function(next) {
          ...
          return async.eachSeries(servers, (function(server, _next) {
            var rsyncCmd;
            rsyncCmd = project.rsyncCmd || "rsync -a --delete-after --ignore-errors --force " + ("" + localDir + "/" + project.name + " " + server[0] + "@" + server[1] + ":" + project.destination);
